...
def ask_question(
    ...
    Args:
        ...
        retriever:         Configured UnifiedRetriever.
        gen_service:       Configured GenerationService.
        slots:             Pre-filled slot dict (used in tests / multi-turn flows).
                           Keys used: ``"age_band"``.
        chat_history:      Optional sliding window conversation history.
        skip_llm_rewriter: If True, bypass LLM call during query rewriting.
        interactive:       If True, prompt stdin interactively for missing clarification slots.
    """
    if slots is None:
        slots = {}
    ...
    print(f"[DEBUG] category={intent.category} refuse={intent.requires_refusal} reason={intent.refusal_reason!r}")

    # ── 1. Hard Refusal / Safety Bypass Pathway ─────────────────────────────
    ...
    # ── 2. Mandatory Age Slot Guard (deterministic, not LLM-controlled) ─────
    ...
    # ── 3. Adaptive Clarification Prompting (non-age slots) ───────────────────
    ...
    # ── 4. Build enriched query string for retrieval ─────────────────────────
    extra_context = []
    ...
    # ── 5. Sub-question decomposition → retrieval → generation ──────────────
