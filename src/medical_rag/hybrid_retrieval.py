...
# Query normalization is applied before BM25/dense search (Phase 2 Workstream B)
try:
    from .query_normalization import normalize_medical_query as _normalize_query
except ImportError:
    def _normalize_query(q: str) -> str:  # type: ignore[misc]
        return q
...
def parse_query_age(query: str) -> tuple[str, bool]:
    """Detect age group from user query string.

    Returns (age_group_label, is_under_6_flag).
    """
    q = query.lower()
    under_6_patterns = [
        r"\b(toddler|infant|baby|preschooler)\b",
        r"\b[1-5]\s*(-|\s*to\s*)?(year|yr|yo|y/o)\b",
        r"\bunder\s+6\b",
        r"\b<6\s*(years?|yr)?\b",
    ]
    if any(re.search(p, q) for p in under_6_patterns):
        return "children_under_6", True

    child_6_11_patterns = [
        r"\b(6|7|8|9|10|11)\s*(-|\s*to\s*)?(year|yr|yo|y/o)\b",
        r"\b6\s*[-–]\s*11\s*(years?|yr)?\b",
        r"\bchild(ren)?\s+(6|7|8|9|10|11)\b",
    ]
    if any(re.search(p, q) for p in child_6_11_patterns):
        return "children_6_11", False

    adult_patterns = [
        r"\b(adults?|adolescents?|teenagers?|teens?|12\+|>=12)\b",
        r"\b(1[2-9]|[2-9]\d)\s*(-|\s*to\s*)?(year|yr|yo|y/o)\b",
    ]
    if any(re.search(p, q) for p in adult_patterns):
        return "adults_adolescents", False

    return "all_ages", False
...
class UnifiedRetriever:
    ...
    def search(
        ...
        # Phase 2: apply medical query normalization before any retrieval
        preprocessed = _normalize_query(query) if normalize else query
        search_query = expand_medical_query(preprocessed) if expand_acronyms else preprocessed

        age_label, is_under_6 = parse_query_age(query)
        ...
        # Filter out sources that do not cover children under 6 if query is under-6 specific
        if is_under_6 and results:
            filtered = [r for r in results if r.metadata.get("covers_under_6", True) is not False]
            if filtered:
                results = filtered

        # Boost results matching query age group
        if age_label in ("children_6_11", "adults_adolescents") and results:
            boosted = []
            for r in results:
                b_score = r.score * 1.15 if r.metadata.get("age_group") == age_label else r.score
                boosted.append(SearchResult(rank=r.rank, score=b_score, text=r.text, metadata=r.metadata))
            boosted.sort(key=lambda x: x.score, reverse=True)
            results = [
                SearchResult(rank=new_rank, score=r.score, text=r.text, metadata=r.metadata)
                for new_rank, r in enumerate(boosted, start=1)
            ]
