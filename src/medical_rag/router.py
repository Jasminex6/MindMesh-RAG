"""Query router — the single entry point every query passes through before retrieval.
...
Flow implemented here:
    ...
    query
      -> safety gate (emergency / injection / out-of-scope / patient scenario)
      -> ambiguity check
...
from .safety import run_safety_gate, SafetyFlag, IntentCategory
...
def classify_ambiguity(query: str) -> tuple[bool, Optional[str]]:
    ...
    if _word_count(stripped) <= 2 and not _mentions_any(
        stripped,
        ["asthma", "ربو", "wheez", "أزيز", "inhaler", "بخاخ", "symptom", "symptoms"],
    ):
        return True, (
...
class RoutingDecision:
    ...
    query: str
    category: str = "NONE"          # "EMERGENCY" | "INJECTION" | "OUT_OF_SCOPE" | "PATIENT_SCENARIO" | "AMBIGUOUS"
...
def route_query(query: str) -> RoutingDecision:
    ...
    Callers should check `status` and act accordingly:
