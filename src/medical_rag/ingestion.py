...
COPYRIGHT_BOILERPLATE_RE = re.compile(
    r"^(?:copyrighted\s+material|do\s+not\s+copy\s+or\s+distribute|gina\s+\d{4}\s+summary\s+guide|global\s+initiative\s+for\s+asthma).*",
    re.IGNORECASE,
)
NAVIGATION_LINES = {
    "back to top",
...
class IngestionService:
    ...
    def clean_page(
        ...
        for index, original in enumerate(lines):
            ...
            elif COPYRIGHT_BOILERPLATE_RE.search(line):
                reason = "copyright_boilerplate"
